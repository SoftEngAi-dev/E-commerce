...
export async function enqueueJob(db:PostgresDatabase,input:{type:string;payload:unknown;maxAttempts?:number;availableAt?:Date}){
  const r=await db.query<{id:string}>(
    "INSERT INTO jobs(type,payload,status,max_attempts,available_at) VALUES($1,$2,'queued',$3,$4) RETURNING id",
    [input.type,input.payload,input.maxAttempts??5,input.availableAt??new Date()]
  );
  return r.rows[0]?.id;
}
...
export async function claimJobs(db:PostgresDatabase,workerId:string,limit=10){
  ...
  UPDATE jobs j SET status='running',attempts=j.attempts+1,locked_at=now(),worker_id=$2
