...
export async function getOrderVersion(db:PostgresDatabase,id:string){
  const r=await db.query<{status:OrderState;version:number}>("SELECT status,version FROM orders WHERE id=$1",[id]);
  return r.rows[0];
}
...
export async function transitionPersistedOrder(db:PostgresDatabase,id:string,to:OrderState,expectedVersion:number,externalPaymentId?:string){
  return db.transaction(async c=>{
    ...
    if((to==="cancelled"||to==="expired"||to==="refunded")&&(current.status==="pending_payment"||current.status==="paid")){
      const reservations=await c.query<{product_id:string;quantity:number}>("SELECT product_id,quantity FROM inventory_reservations WHERE order_id=$1 AND status='reserved' FOR UPDATE",[id]);
      for(const item of reservations.rows){
        await c.query("UPDATE products SET reserved_stock=reserved_stock-$1,updated_at=now() WHERE id=$2 AND reserved_stock>=$1",[item.quantity,item.product_id]);
        await c.query("UPDATE inventory_reservations SET status='released' WHERE order_id=$1 AND product_id=$2 AND status='reserved'",[id,item.product_id]);
      }
    }
    ...
    if(to==="submitted"&&current.status==="paid"){
      const reservations=await c.query<{product_id:string;quantity:number}>("SELECT product_id,quantity FROM inventory_reservations WHERE order_id=$1 AND status='reserved' FOR UPDATE",[id]);
      for(const item of reservations.rows){
        const updated=await c.query("UPDATE products SET stock=stock-$1,reserved_stock=reserved_stock-$1,updated_at=now() WHERE id=$2 AND stock>=$1 AND reserved_stock>=$1 RETURNING id",[item.quantity,item.product_id]);
        if(updated.rowCount!==1)throw new Error("Insufficient stock at fulfillment commit");
        await c.query("UPDATE inventory_reservations SET status='committed' WHERE order_id=$1 AND product_id=$2 AND status='reserved'",[id,item.product_id]);
      }
    }
    ...
    await c.query("INSERT INTO outbox_events(topic,aggregate_type,aggregate_id,payload) VALUES($1,$2,$3,$4)",["order.status.changed","order",id,JSON.stringify({orderId:id,status:to,version:current.version+1})]);
...
export async function getOrder(db:PostgresDatabase,id:string){
  ...
  const items=await db.query('SELECT product_id AS "productId",external_id AS "externalId",title_snapshot AS "title",quantity,unit_price AS "unitPrice" FROM order_items WHERE order_id=$1 ORDER BY id',[id]);
