...
export async function createOrder(db:PostgresDatabase,input:CreateOrderInput){
  return db.transaction(async c=>{
    const existing=await c.query<{id:string;status:OrderState;total:number}>(
      "SELECT id,status,total FROM orders WHERE idempotency_key=$1",[input.idempotencyKey]
    );
    if(existing.rows[0])return existing.rows[0];
    await c.query(
      "INSERT INTO orders(id,status,currency,subtotal,shipping,total,idempotency_key,customer_id,shipping_address) VALUES($1,'pending_payment',$2,$3,$4,$5,$6,$7,$8)",
      [input.id,input.currency,input.subtotal,input.shipping,input.total,input.idempotencyKey,input.customerId,input.shippingAddress]
    );
...
export async function transitionPersistedOrder(
  db:PostgresDatabase,id:string,to:OrderState,expectedVersion:number,externalPaymentId?:string
){
  return db.transaction(async c=>{
    ...
    await c.query(
      ...
      ["order.status.changed","order",id,JSON.stringify({orderId:id,status:to})]
    );
...
export async function getOrder(db:PostgresDatabase,id:string){
  ...
  const items=await db.query(
    'SELECT product_id AS "productId",external_id AS "externalId",title_snapshot AS "title",quantity,unit_price AS "unitPrice",unit_cost AS "unitCost",shipping_cost AS "shippingCost" FROM order_items WHERE order_id=$1 ORDER BY id',
    [id]
  );
