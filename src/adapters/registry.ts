...
export class ProviderRegistry{
  private productSources=new Map<string,ProductSource>();private payments=new Map<string,PaymentProvider>();private fulfillment=new Map<string,FulfillmentProvider>();
  ...
  addFulfillment(p:FulfillmentProvider){this.fulfillment.set(p.id,p);return this}
  ...
  fulfillment(id:string){const p=this.fulfillment.get(id);if(!p)throw new Error("Unknown fulfillment provider: "+id);return p}
}
