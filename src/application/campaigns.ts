...
export function validateCampaignBudget(budget:number){
  if(!Number.isFinite(budget)||budget<0)throw new Error("Invalid campaign budget");
}

export function campaignRisk(draft:CampaignDraft):ActionContext{
  validateCampaignBudget(draft.budget);
  const risk:"low"|"medium"|"high"=draft.budget>1000?"high":draft.budget>250?"medium":"low";
  return{
    action:"campaign.create",
    risk,
    evidence:["budget-policy","market-policy","channel-policy"]
  };
}
