...
import { getOrder, transitionPersistedOrder } from "../persistence/order-pg.js";
import { enqueueJob } from "../persistence/job-queue-pg.js";

export async function submitPaidOrder(db:PostgresDatabase,provider:FulfillmentProvider,orderId:string,idempotencyKey:string){
  ...
  const fulfillment=await provider.submit({orderId,items,address:JSON.stringify(address),idempotencyKey});
  ...
  await enqueueJob(db,{
    type:"fulfillment.tracking.sync",
    payload:{externalFulfillmentId:fulfillment.externalId},
    maxAttempts:20,
